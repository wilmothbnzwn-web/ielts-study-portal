...
-- ================================================================
-- 6. Mistake Book (Wrong Questions) — 错题本
-- ================================================================
...
-- Indexes for fast per-user queries and filtering
...
-- SELECT: users can only view their own wrong questions
CREATE POLICY "Users can view own wrong questions"
  ON public.wrong_questions FOR SELECT
  USING (auth.uid() = user_id);

-- INSERT: client must set user_id = auth.uid()
